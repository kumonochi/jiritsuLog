class JiritsuLogApp {
    constructor() {
        ...
        // Google認証設定
        this.GOOGLE_CLIENT_ID = '47690741133-c4pbiefj90me73dflkla5q3ie67nbqdl.apps.googleusercontent.com'; // 実際の値に置き換える必要があります
        ...
        this.environment = { supportedByGoogleAuth: true };
    ...
    setupEventListeners() {
        ...
        // Google認証関連のイベントリスナー
        this.setupGoogleAuthEventListeners();
    }
    ...
    async handleGoogleSignIn() {
        try {
            this.debugLog('Googleサインイン開始');
            
            if (typeof google === 'undefined' || !google.accounts) {
                this.errorLog('Google Identity Services が読み込まれていません');
                return;
            }
            ...
            if (user) {
                ...
                this.updateAuthUI();
                this.showPopupNotification('Googleアカウントに連携しました', 'success');
    ...
    async handleGoogleSignOut() {
        try {
            ...
            // ローカルデータをユーザー固有から共通に戻す
            this.relocateUserData(false);
            
            this.showPopupNotification('Googleアカウントの連携を解除しました', 'info');
            
        } catch (error) {
            this.errorLog('サインアウトエラー:', error);
        }
    }
    ...
    // ユーザーデータの移動（サインイン・アウト時）
    relocateUserData(toUserSpecific) {
        try {
            if (toUserSpecific && this.currentUser) {
                // 共通データをユーザー固有に移動
                const commonRecordsKey = 'jiritsulog_records';
                const commonSettingsKey = 'jiritsulog_settings';
                
                const userRecordsKey = this.getStorageKey('records');
                const userSettingsKey = this.getStorageKey('settings');
                
                // データを移動
                const commonRecords = localStorage.getItem(commonRecordsKey);
                const commonSettings = localStorage.getItem(commonSettingsKey);
                
                if (commonRecords) {
                    localStorage.setItem(userRecordsKey, commonRecords);
                    localStorage.removeItem(commonRecordsKey);
                }
                
                if (commonSettings) {
                    localStorage.setItem(userSettingsKey, commonSettings);
                    localStorage.removeItem(commonSettingsKey);
                }
                ...
                if (userRecords) {
                    localStorage.setItem(commonRecordsKey, userRecords);
                    localStorage.removeItem(userRecordsKey);
                }
                
                if (userSettings) {
                    localStorage.setItem(commonSettingsKey, userSettings);
    ...
    // ストレージキーを生成（アカウント連携対応）
    getStorageKey(type) {
        const userPrefix = this.currentUser ? `user_${this.currentUser.email}_` : '';
        const baseKey = `jiritsulog_${userPrefix}${type}`;
        this.debugLog(`ストレージキー生成: ${baseKey}`);
        return baseKey;
    }
    
    // Google Drive APIを使ったデータ同期
    async syncDataWithGoogle() {
        if (!this.isSignedIn) {
            this.debugLog('未サインインのため同期をスキップ');
            return;
        }
        
        try {
            this.debugLog('Google Driveとのデータ同期を開始');
            ...
            await this.uploadToGoogleDrive(currentData);
            
            // 最終同期時刻を更新
            this.updateLastSyncTime();
            this.addSyncHistory('自動同期', true, 'データをクラウドに保存しました');
            
        } catch (error) {
            this.errorLog('データ同期エラー:', error);
            this.addSyncHistory('自動同期', false, `同期エラー: ${error.message}`);
        }
    }
    
    // Google Driveにデータをアップロード
    async uploadToGoogleDrive(data) {
        const fileName = `jiritsulog_backup_${this.currentUser.email.replace('@', '_at_')}.json`;
        const fileData = JSON.stringify(data, null, 2);
        
        // 既存ファイルをチェック
        const existingFile = await this.findFileInDrive(fileName);
        
        if (existingFile) {
            // 既存ファイルを更新
            await this.updateFileInDrive(existingFile.id, fileData);
        } else {
            // 新しいファイルを作成
            await this.createFileInDrive(fileName, fileData);
        }
    }
    ...
    async findFileInDrive(fileName) {
        try {
            ...
            this.errorLog('ファイル検索エラー:', error);
            throw error;
        }
    }
    
    // Google Driveにファイルを作成
    async createFileInDrive(fileName, content) {
    ...
    async updateFileInDrive(fileId, content) {
        const request = gapi.client.request({
    ...
    // Google Driveからデータを同期
    async syncFromGoogleDrive() {
        if (!this.isSignedIn) {
            this.showPopupNotification('Googleアカウントにサインインしてください', 'warning');
            return;
        }
        
        try {
            this.debugLog('Google Driveからのデータ同期を開始');
            
            const fileName = `jiritsulog_backup_${this.currentUser.email.replace('@', '_at_')}.json`;
            const file = await this.findFileInDrive(fileName);
            
            if (!file) {
                this.showPopupNotification('クラウドにバックアップが見つかりません', 'info');
                return;
            }
            ...
            this.loadUserData();
            this.updateLastSyncTime();
            this.addSyncHistory('手動同期', true, 'クラウドからデータを復元しました');
            
            this.showPopupNotification('クラウドからデータを同期しました', 'success');
            
        } catch (error) {
            this.errorLog('同期エラー:', error);
            this.addSyncHistory('手動同期', false, `同期エラー: ${error.message}`);
        }
    }
    ...
    updateLastSyncTime() {
        ...
        if (syncTimeElement) {
            const now = new Date();
            syncTimeElement.textContent = `最終同期: ${now.toLocaleString()}`;
        }
    }

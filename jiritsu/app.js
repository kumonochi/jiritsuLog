class JiritsuLogApp {
    ...
    setupMainGoogleSignin() {
        ...
        const signoutButton = document.getElementById('main-google-signout');
        if (signoutButton) {
    ...
    // HTMLから呼び出される手動Googleサインイン
    handleManualGoogleSignin() {
        console.log('手動Googleサインインが呼び出されました');
        try {
            // 既存のGoogle認証処理を実行
            this.requestAccessToken();
        } catch (error) {
            console.error('手動サインインエラー:', error);
            this.showPopupNotification('Googleサインインに失敗しました', 'error');
        }
    }
    
    // HTMLから呼び出されるGoogleサインアウト
    handleGoogleSignout() {
        console.log('Googleサインアウトが呼び出されました');
        try {
            // サインアウト処理
            this.isSignedIn = false;
            this.currentUser = null;
            ...
            this.updateMainGoogleAuthUI();
            
            this.showPopupNotification('Googleアカウントからサインアウトしました', 'success');
        } catch (error) {
            console.error('サインアウトエラー:', error);
            this.showPopupNotification('サインアウトに失敗しました', 'error');
        }
    }
    
    // メインGoogle認証UIの更新
    updateMainGoogleAuthUI() {
        const signinSection = document.getElementById('main-google-signin-section');
        const userInfoSection = document.getElementById('main-google-user-info');
...
// HTMLから呼び出されるGoogle認証関数をグローバルに追加
window.handleManualGoogleSignin = function() {
    if (app) {
        app.handleManualGoogleSignin();
    }
};

window.handleGoogleSignout = function() {
    if (app) {
        app.handleGoogleSignout();
    }
};
